...
class NafriShader extends FlxShader
{
	@:glFragmentSource('
    #pragma header
    ...
    vec4 bloom_pass(vec2 uv) {
        vec2 texel = 0.015 / openfl_TextureSize;
        ...
        for (float x = -0.5; x <= 0.5; x += 1.0 / samples.x) {
            for (float y = -0.5; y <= 0.5; y += 1.0 / samples.y) {
                ...
                float luminance = dot(tex.rgb, vec3(0.0, 0.0, 0.0));
    ...
    void main() {
        ...
        vec4 bloom = vec4(0.0549, 0.0353, 0.1333, 0.007);
        if (u_brightness > 0.0) {
