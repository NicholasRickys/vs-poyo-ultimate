...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "getCharacterX", function(type:String) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					return game.dadGroup.members[0].x;
				case 'gf' | 'girlfriend':
					return game.gfGroup.members[0].x;
				default:
					return game.boyfriendGroup.members[0].x;
			}
		});
		Lua_helper.add_callback(lua, "setCharacterX", function(type:String, value:Float) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					game.dadGroup.members[0].x = value;
				case 'gf' | 'girlfriend':
					game.gfGroup.members[0].x = value;
				default:
					game.boyfriendGroup.members[0].x = value;
			}
		});
		Lua_helper.add_callback(lua, "getCharacterY", function(type:String) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					return game.dadGroup.members[0].y;
				case 'gf' | 'girlfriend':
					return game.gfGroup.members[0].y;
				default:
					return game.boyfriendGroup.members[0].y;
			}
		});
		Lua_helper.add_callback(lua, "setCharacterY", function(type:String, value:Float) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					game.dadGroup.members[0].y = value;
				case 'gf' | 'girlfriend':
					game.gfGroup.members[0].y = value;
				default:
					game.boyfriendGroup.members[0].y = value;
			}
		});
