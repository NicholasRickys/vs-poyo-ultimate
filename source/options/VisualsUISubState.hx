...
class VisualsUISubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		poyo = new Character(640, -190, 'poyoing', true);
		poyo.setGraphicSize(Std.int(poyo.width * 0.75));
		poyo.updateHitbox();
		poyo.dance();
		poyo.animation.finishCallback = function (name:String) poyo.dance();
		poyo.visible = false;

		old_poyo = new Character(840, 20, 'poyoing-scrapped', true);
		old_poyo.setGraphicSize(Std.int(old_poyo.width * 0.75));
		old_poyo.updateHitbox();
		old_poyo.dance();
		...
		var option:Option = new Option('Poyo Sprites',
			'The reason why we scrapped the Poyo sprites you seen in the catmania video was because nobody was able to keep a consistent artstyle to PennyPancake.
