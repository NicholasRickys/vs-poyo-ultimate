...
class PlayState extends MusicBeatState
{
	...
	public var opponent_songScore:Int = 0;
	...
	override public function create()
	{
		...
		{
			if (SONG.characters == null || SONG.characters != null && SONG.characters.gfVersion == null)
			{
			...
			{
				for (gfData in SONG.characters.gfVersion)
				{
					if (!gfData.main)
					{
						var x:Float = gfData.xdiff;
						if (gfData.xdiff == null)
							x = 0;

						var y:Float = gfData.ydiff;
						if (gfData.ydiff == null)
							y = 0;

						if(gfData.name == null || gfData.name.length < 1) gfData.name = 'gf'; //Fix for the Chart Editor
						var gf = new Character(GF_X + x, GF_Y + y, gfData.name);

						if (gfData.notetype != null)
							gf.notetype = gfData.notetype;
					...
					else
					{
						...
						if(gfData.name == null || gfData.name.length < 1) gfData.name = 'gf'; //Fix for the Chart Editor
						gf = new Character(GF_X + x, GF_Y + y, gfData.name);
						startCharacterPos(gf);
						gf.scrollFactor.set(0.95, 0.95);
						gfGroup.add(gf);
						startCharacterScripts(gf.curCharacter);
		...
		else
		{
			for (dadData in SONG.characters.player2)
			{
				...
				if (!dadData.main)
				{
					var x:Float = dadData.xdiff;
					if (dadData.xdiff == null)
						x = 0;

					var y:Float = dadData.ydiff;
		...
		else
		{
			for (bfData in SONG.characters.player1)
			{
				...
				if (!bfData.main)
				{
					...
					if (bfData.xdiff == null)
						x = 0;

					var y:Float = bfData.ydiff;
					...
					startCharacterScripts(bf.curCharacter);
				}
				else
				{
					var x:Float = bfData.xdiff;
					...
					startCharacterPos(boyfriend);
					boyfriendGroup.add(boyfriend);
					startCharacterScripts(boyfriendGroup.members[0].curCharacter);
				}
			}
		}
	...
	public function reloadHealthBarColors() {
		healthBar.setColors(FlxColor.fromRGB(dadGroup.members[0].healthColorArray[0], dadGroup.members[0].healthColorArray[1], dadGroup.members[0].healthColorArray[2]),
	...
	public function startCountdown()
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			{
				gfGroup.forEachAlive(function(gf)
	...
	public function updateScore(miss:Bool = false)
	{
		...
		+ ' | Opponent Score: ' + opponent_songScore
		+ ' | Misses: ' + songMisses
		+ ' | Rating: ' + str;
	...
	override public function update(elapsed:Float)
	{
		...
		if(!inCutscene && !paused) {
			...
			if(!startingSong && !endingSong && boyfriendGroup.members[0].animation.curAnim != null && boyfriendGroup.members[0].animation.curAnim.name.startsWith('idle')) {
		...
		if (generatedMusic)
		{
			if(!inCutscene)
			{
				if(!cpuControlled) {
					...
					boyfriendGroup.forEachAlive(function(boyfriend)
						if(boyfriend != null && boyfriend.animation.curAnim != null && boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * boyfriend.singDuration && boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss'))
							boyfriend.dance());
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				boyfriendGroup.members[0].stunned = true;
				deathCounter++;
				...
				#end
				openSubState(new GameOverSubstate(boyfriendGroup.members[0].getScreenPosition().x - boyfriendGroup.members[0].positionArray[0], boyfriendGroup.members[0].getScreenPosition().y - boyfriendGroup.members[0].positionArray[1], camFollow.x, camFollow.y));
		...
		switch(eventName) {
			case 'Hey!':
				...
				if(value != 1) {
					...
					boyfriendGroup.members[0].heyTimer = flValue2;
				}
			...
			case 'Change Character':
				...
				switch(charType) {
					case 0:
						if(boyfriendGroup.members[0].curCharacter != value2) {
							if(!boyfriendMap.exists(value2)) {
								addCharacterToList(value2, charType);
							}

							var lastAlpha:Float = boyfriendGroup.members[0].alpha;
							boyfriendGroup.members[0].alpha = 0.00001;
							boyfriendGroup.members[0] = boyfriendMap.get(value2);
							boyfriendGroup.members[0].alpha = lastAlpha;
							iconP1.changeIcon(boyfriendGroup.members[0].healthIcon);
						}
						setOnScripts('boyfriendName', boyfriendGroup.members[0].curCharacter);
	...
	public function moveCamera(isDad:Bool)
	{
		...
		{
			camFollow.setPosition(boyfriendGroup.members[0].getMidpoint().x - 100, boyfriendGroup.members[0].getMidpoint().y - 100);
	...
	public function endSong()
	{
		...
		if(opponent_songScore > songScore)
		{
			persistentUpdate = false;
			persistentDraw = false;
			#if LUA_ALLOWED
	...
	private function popUpImage(player:Bool, x:Float, y:Float, uiPrefix:String, uiSuffix:String, antialias:Bool, daRating:Rating)
	{
		var rating:FlxSprite = new FlxSprite();
		var placement:Float = x;
		var rating:FlxSprite = new FlxSprite(placement, y);
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		var character:Array<Character> = [boyfriend];
		var hasResetted:Bool = false;
		...
		if((note != null && note.gfNote) || (SONG.notes[curSection] != null && SONG.notes[curSection].gfSection)) character = [gf];
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			var character:Array<Character> = [dad];
			var hasResetted:Bool = false;
	...
	public function spawnNoteSplashOnNote(note:Note) {
		if(note != null) {
			...
			var strum:StrumNote = strumLineNotes.members[note.noteData + thing];
			if(strum != null)
